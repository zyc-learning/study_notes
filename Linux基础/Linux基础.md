...
在命令模式下按下`:`（英文冒号）就进入了编辑模式。

编辑模式可以输入单个或多个字符的命令，可用的命令非常多。
...
**:open filename**:在vim窗口打开一个新文件 :open!放弃当前文件的修改，重新打开未修改的文件

**:w [path/]filename/:saveesa [path/]filename**：另存为
...
**:sspeend/Crtl + Z**:将vim挂起，回到shell，使用fg可以返回vim
...
| d0/d^    | 那个是数字的 0 ，删除光标所在处，到该行的最前面一个字符      |
| dw       | 删除当前字符到单词尾                                         |
| daw      | 删除当前字符所在单词                                         |
| yy       | 复制游标所在的那一行(常用)                                   |
...
| :set ingrorecase  | 忽略大小写的查找                                   |
| :set noigrorecase | 不忽略大小写的查找                                 |
| :ce               | 居中                                               |
| :le               | 靠左                                               |
| :ri               | 靠右                                               |
